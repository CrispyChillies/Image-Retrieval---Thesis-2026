    ...
    for i, batch in enumerate(data_loader):
        ...
        if i == 0 and epoch == 1 and rank == 0:
            ...
            if hasattr(raw_model.model, "logit_scale"):
                ...
                print(
                    f"Initial logit_scale value: {ls.item():.4f} (exp={ls.exp().item():.4f})"
                )
        ...
        with autocast("cuda", enabled=use_amp):
            ...
            if not torch.isfinite(total_loss):
                print(
                    f"[ERROR] Non-finite loss at iter {i}, epoch {epoch}: {total_loss.item()}"
                )
        ...
        with torch.no_grad():
            raw_model = model.module if hasattr(model, "module") else model
            if hasattr(raw_model.model, "logit_scale"):
                raw_model.model.logit_scale.clamp_(
                    0.0, 4.6052
                )  # [1, 100] temperature range
            ...
            if rank == 0:
                ...
                if n % 50 == 0:  # Every 50 iterations instead of every 10
                    ...
                    if hasattr(raw_model_log.model, "logit_scale"):
                        ...
                        temp_val = (
                            2.71828**ls_val
                        )  # Use constant instead of torch.tensor().exp()
                        print(
                            f"   [Info] logit_scale={ls_val:.4f}, temperature={temp_val:.2f}"
                        )
            running_loss = 0
            running_it_loss = 0
            running_rc_loss = 0
...
def evaluate_conceptclip(model, loader, device, rank=0, world_size=1):
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        if batch_idx == 0:
            first_batch_embeds_for_debug = (
                embedding[: min(3, len(embedding))].detach().cpu()
            )
...
def main(args):
    ...
    if args.model == "conceptclip":
        ...
        if rank == 0:
            print(
                f"ConceptCLIP: {trainable_count:,} / {total_count:,} parameters trainable "
        ...
        hasattr(model.module if args.use_ddp else model, "f1")
        and hasattr(model.module if args.use_ddp else model, "f2")
        and hasattr(model.module if args.use_ddp else model, "attention")
    ):
        ...
        optimizer = Adam(
            [
                ...
                {
                    "params": model_without_ddp.attention.parameters(),
                    "lr": args.lr * 0.01,
                },
        ...
        if rank == 0:
            print(
                f"DINOv2: {trainable_count:,} / {total_count:,} parameters trainable "
                f"({100*trainable_count/total_count:.1f}%)"
            )
...
def parse_args():
    ...
    parser.add_argument(
        "--dataset-dir",
        default="/data/brian.hu/COVID/data/",
    ...
    parser.add_argument(
        "--anomaly", action="store_true", help="Train without anomaly class"
    )
    parser.add_argument(
        "--model",
        default="densenet121",
    ...
    parser.add_argument(
        ...
        default=3,
        type=int,
        help="Number of final DINOv2 transformer blocks to unfreeze",
    )
    parser.add_argument(
        "-p",
        "--labels-per-batch",
    ...
    parser.add_argument(
        "--eval-freq", default=2, type=int, help="Evaluate model every N epochs"
    )
    parser.add_argument(
        "-j",
        "--workers",
    ...
    parser.add_argument(
        "--save-dir", default="./checkpoints", help="Model save directory"
    )
    ...
    parser.add_argument(
        "--use-ddp", action="store_true", help="Use Distributed Data Parallel training"
    )
    ...
    parser.add_argument(
        ...
        default=0.01,
        type=float,
        help="Weight decay for AdamW optimizer (ConceptCLIP/DINOv2)",
    )
    parser.add_argument(
        "--amp",
        action="store_true",
