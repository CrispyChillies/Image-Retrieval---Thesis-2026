...
MODEL_CONFIGS = {
    ...
    "resnet50": {
        "embedding_dim": 2048,
        "collection_names": {
            "default": "image_retrieval_resnet50",
            "isic": "isic_image_retrieval_resnet50",
            "covid": "covid_image_retrieval_resnet50",
        },
    ...
    "convnextv2": {
        "embedding_dim": 1024,
        "collection_names": {
            "default": "image_retrieval_convnextv2",
            "isic": "isic_image_retrieval_convnextv2",
    ...
    "medsiglip": {
        "embedding_dim": 512,
        "collection_names": {
            "default": "isic_image_retrieval_medsiglip",
            "isic": "isic_image_retrieval_medsiglip",
            "covid": "covid_image_retrieval_medsiglip",
        },
...
class MilvusManager:
    ...
    def __init__(self, uri=None, token=None, user="", password="", dataset="default"):
        """
    ...
    def get_model_config(self, model_type):
        """Resolve model config for the selected dataset."""
        if model_type not in MODEL_CONFIGS:
            raise ValueError(f"Unknown model type: {model_type}")

        config = MODEL_CONFIGS[model_type].copy()
        collection_names = config.pop("collection_names", {})
        config["collection_name"] = collection_names.get(
            self.dataset, collection_names.get("default")
        )

        if not config["collection_name"]:
            raise ValueError(
                f"No collection name configured for model '{model_type}' "
                f"and dataset '{self.dataset}'"
            )
    ...
    def create_index(
        self, model_type, index_type="IVF_FLAT", metric_type="COSINE", nlist=1024
    ):
        """
...
def main():
    ...
    parser.add_argument(
        "--drop_old", action="store_true", help="Drop existing collections"
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="default",
        choices=["default", "covid", "isic"],
        help="Dataset-specific collection naming",
    )
    parser.add_argument(
        "--metric_type",
        type=str,
    ...
    # Create manager
    manager = MilvusManager(uri=args.uri, token=args.token, dataset=args.dataset)
    ...
    try:
        ...
        if args.model == "all":
            manager.setup_all_collections(
                drop_old=args.drop_old, metric_type=args.metric_type
            )
        else:
